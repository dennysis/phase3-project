...
def manage_patients():
    ...
    while True:
        ...
        if choice == 1:
            ...
            heartbeat = click.prompt("Enter patient's heartbeat frequency (Hz)", type=int)
            add_patient(name, age, heartbeat)
        elif choice == 2:
...
def add_patient(name, age, heartbeat):
    """Add a new patient."""
    try:
        patient = Patient(name=name, age=age, heartbeat=heartbeat, status='admitted')
        session.add(patient)
        session.commit()
...
def list_patients():
    ...
    try:
        ...
        for patient in patients:
            click.echo(f'{patient.id}: {patient.name}, {patient.age} years old, {patient.heartbeat} Hz, Status: {patient.status}')
    except Exception as e:
